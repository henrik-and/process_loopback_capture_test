...
void CLoopbackCapture::MuteCapturedProcess(DWORD captured_process_id, bool muteEndpoint)
{
    ...
    HRESULT hr = CoCreateInstance(
        ...
        CLSCTX_ALL, IID_MMDeviceEnumerator,
    ...
    UINT n_devices;
    hr = device_collection->GetCount(&n_devices);
    if (hr != S_OK) {
        return;
    }
    ...
    for (UINT i = 0; i < n_devices; i++) {
        ...
        IAudioSessionManager2* audio_session_manager_2 = nullptr;
        hr = device->Activate(
            __uuidof(IAudioSessionManager2), CLSCTX_ALL,
            NULL, (void**)&audio_session_manager_2
        );
        if (hr != S_OK) {
            continue;
        }

        IAudioSessionEnumerator* audio_session_enumerator = nullptr;
        hr = audio_session_manager_2->GetSessionEnumerator(&audio_session_enumerator);
        if (hr != S_OK) {
            continue;
        }

        int n_sessions;
        hr = audio_session_enumerator->GetCount(&n_sessions);
        if (hr != S_OK) {
            continue;
        }
        ...
        for (int j = 0; j < n_sessions; j++) {
            hr = audio_session_enumerator->GetSession(j, &audio_session_control);
            if (hr != S_OK) {
                std::wcout << L"Failed to enumerate session." << std::endl;
                continue;
            }
            
            hr = audio_session_control->GetDisplayName(&pswSession);
            if (hr != S_OK) {
                std::wcout << L"Failed to get session's display name." << std::endl;
                continue;
            }
            std::wcout << L"Session: " << pswSession << std::endl;

            IAudioSessionControl2* audio_session_control_2 = nullptr;
            hr = audio_session_control->QueryInterface(&audio_session_control_2);
            if (hr != S_OK) {
                std::wcout << L"Failed to query IAudioSessionControl2 from IAudioSessionControl." << std::endl;
                continue;
            }
            
            hr = audio_session_control_2->GetProcessId(&process_id);
            if (hr != S_OK) {
                std::wcout << L"Failed to obtain process id." << std::endl;
                continue;
            }

            hr = audio_session_control_2->GetSessionIdentifier(&session_id);
            if (hr != S_OK) {
                std::wcout << L"Failed to obtain session id." << std::endl;
                continue;
            }
